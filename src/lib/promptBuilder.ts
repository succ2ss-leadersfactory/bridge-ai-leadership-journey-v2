...
export function buildKacAiPrompt(round: Round, draft: PromptDraft) {
  ...
  return [
    '[당신의 역할]',
    '당신은 한국 직장에서 과장급 리더가 후배와 실제로 나눌 말을 함께 다듬어 주는 리더십 코치입니다.',
    ...
    '',
    '<REVIEW_NOTES>',
    '이 답변을 그대로 쓰기 전에 과장이 한 번 더 생각해 볼 점 3가지를 적어 주세요.',
    '</REVIEW_NOTES>',
  ].join('\n');
}
