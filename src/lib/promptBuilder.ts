...
export function buildKacAiPrompt(round: Round, draft: PromptDraft, pathContext: PromptPathContext = {}) {
  ...
  return [
    ...
    '',
    '[그 선택이 만든 변화: 업무 흐름과 남은 부담]',
    getFirstChoiceResult(round, draft),
    '',
    '[후배가 받아들인 의미: 해석과 다음 행동 가능성]',
    juniorReaction,
    '',
    '[처음 선택의 비용이 드러난 장면]',
    additionalSituation,
    ...
    '',
    '[답변을 만들 때 반드시 구분할 것]',
    '- “그 선택이 만든 변화”는 업무 흐름, 즉시 결과, 남은 실무 부담으로만 해석해 주세요.',
    '- “후배가 받아들인 의미”는 후배의 감정, 해석, 다음 행동 가능성으로만 해석해 주세요.',
    '- “처음 선택의 비용이 드러난 장면”은 방금 선택이 틀렸다는 뜻이 아니라, 시간이 지나며 생긴 다른 부담으로 봐 주세요.',
    '- “다시 봐야 할 판단 지점”은 김원중 과장이 무엇을 유지하고, 무엇을 보완하거나 전환해야 하는지로 정리해 주세요.',
    '- 답변 안에서 사건 설명과 후배 심리 해석을 섞지 말고, 실행안과 대화문에 각각 다르게 반영해 주세요.',
    '',
    '[2주 실행안 5개 항목 작성 기준]',
    ...
    '- 3번과 4번은 같은 뜻으로 쓰지 마세요. 3번은 후배의 행동, 4번은 리더의 개입 범위입니다.',
    '- 1문장은 입 밖으로 내면 후배가 방어적으로 들을 수 있는 표현을 피하는 기준으로 써 주세요.',
    ...
    `1. ${finalOutputName}`,
    '- 2주 뒤 달라졌으면 하는 모습:',
    '- 이번 주에 맡겨볼 작은 일: 후배가 이번 주 안에 직접 해볼 1개의 작은 행동만 씁니다.',
    '- 김원중 과장이 옆에서 도와줄 일: 리더가 어디까지 지원하고 어떤 기준을 같이 볼지만 씁니다.',
    '- 언제 짧게 같이 볼지:',
    '- 말할 때 조심할 표현:',
    '',
    '2. 후배 코칭 대화문 5줄',
    '1) 먼저 풀어줄 말:',
    '2) 바로 답하기 전에 물어볼 말:',
    '3) 이번 주에 같이 해볼 일: 후배가 직접 실행할 이번 주 행동만 씁니다.',
    '4) 김원중 과장이 봐줄 선: 리더가 개입할 범위와 후배가 먼저 해볼 범위를 구분해서 씁니다.',
    '5) 입 밖으로 내면 안 좋은 말:',
    '</FINAL_ARTIFACT>',
    '',
    '<REVIEW_NOTES>',
